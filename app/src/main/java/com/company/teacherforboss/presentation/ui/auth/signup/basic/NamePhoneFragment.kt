            ...
            nameBox.addTextChangedListener(object: TextWatcher {
                ...
                override fun afterTextChanged(s: Editable?) {
                    ...
                    if (noSpacesText != s.toString()) {
                        nameBox.setText(noSpacesText)
                        nameBox.setSelection(noSpacesText?.length ?: 0)
                    }
                }
            })

            phoneNumBox.addTextChangedListener(object: TextWatcher {
                override fun beforeTextChanged(s: CharSequence?, start: Int, count: Int, after: Int) {}
                override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {}
                override fun afterTextChanged(s: Editable?) {
                    val noSpacesText = s?.toString()?.replace("\\s".toRegex(), "")
                    if (noSpacesText != s.toString()) {
                        phoneNumBox.setText(noSpacesText)
                        phoneNumBox.setSelection(noSpacesText?.length ?: 0)
                    }
                }
            })

            phoneCodeBox.addTextChangedListener(object: TextWatcher {
                override fun beforeTextChanged(s: CharSequence?, start: Int, count: Int, after: Int) {}
                override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {}
                override fun afterTextChanged(s: Editable?) {
                    val noSpacesText = s?.toString()?.replace("\\s".toRegex(), "")
                    if (noSpacesText != s.toString()) {
                        phoneCodeBox.setText(noSpacesText)
                        phoneCodeBox.setSelection(noSpacesText?.length ?: 0)
                    }
                }
            })
        }
